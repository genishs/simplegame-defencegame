...
**Phase 4 자동 가드 완결 — `v0.4.0-rc.8` 사전 릴리즈 (2026-05-21, 사용자 .exe 검수용 RC, develop@72dcca6). rc.7 검수 결함 4건(#73~#76) 종합 fix — 영웅 평타 호밍(명중 보장) + 성문 HP HUD 게이지 + 튜토리얼 전장 backdrop + 단계 5 영웅 이동 시연. **핵심 발견**: #75(성 hp 체감 안 됨)는 lives 차감(#62) 정상이고 HUD 표시 부재가 원인 — #71과 동일 패턴. #76은 발사체 좌표 고정이라 이동 적 빗나감 → 호밍 전환. pytest **554**. `v0.4.0` GA는 CAT-01~07 + DPI 매트릭스 최종 통과 후 별도 SCM 라운드에서 결정 (DECISION-SCM-P5K-001/003~010).**
